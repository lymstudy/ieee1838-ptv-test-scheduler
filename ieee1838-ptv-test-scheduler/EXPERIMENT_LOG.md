...
## Design/Prototype Note 024: B3.1.5 Layered Schedule Metrics/Evaluator
...
```bash
pytest tests/test_layered_evaluator.py
pytest
python experiments/demo_layered_schedule_metrics.py --output-dir tmp_layered_schedule_metrics_out
```

Purpose:
Add lightweight metrics/evaluation support for schedules produced by the B3.1 `AccessTimeAwareScheduler` without modifying existing A0 task-level schedulers, evaluator modules, AccessPath logic, or B2 layered expansion logic.

Files added:

```text
src/layered/evaluator.py
tests/test_layered_evaluator.py
experiments/demo_layered_schedule_metrics.py
```
...
Implemented behavior:
- `LayeredScheduleMetrics` stores total time, phase count, PTAP/FPP/DWR/capture/local busy times, access overhead, parallelism metrics, resource busy time, and phase type time.
- `LayeredScheduleEvaluator.evaluate()` consumes `LayeredScheduleResult` and computes phase-level access/resource metrics.
- FPP utilization uses lane-seconds divided by available lane-seconds.
- Access overhead excludes `LOCAL_BIST_RUN` and other local execution phases.
- Max and average parallelism are computed from scheduled phase start/end intervals.

Demo outputs:

```text
tmp_layered_schedule_metrics_out/metrics_summary.csv
tmp_layered_schedule_metrics_out/metrics_summary.md
```

Key validation observations:
- Empty schedules evaluate to zero metrics.
- PTAP and FPP utilization are computed from busy time and lane capacity.
- Local execution time is separated from PTAP busy time.
- Access overhead excludes local BIST execution.
- Parallelism metrics detect overlapping phase intervals.
- Phase type time aggregates durations by `phase_type`.
...
Notes:
B3.1.5 evaluates phase-level access/resource behavior only. It does not implement thermal or voltage prediction, does not implement predictive physical-aware scheduling, and does not claim full IEEE 1838 compliance or a complete IEEE 1838 framework.

## Maintenance Note 025: Repository Cleanup
...
Purpose:
Remove clearly unnecessary repository clutter while preserving core project files, source code, configs, tests, and official `results/` artifacts.
...
Files removed or marked deleted:

```text
tmp_access_path_out/*
tmp_access_time_scheduler_out/*
tmp_benchmark_out/*
tmp_layered_out/*
tmp_layered_schedule_metrics_out/*
RESULTS.md
docs/NEXT_PHASE_PLAN.md
docs/FRONTIER_IDEA_INTEGRATION_PLAN.md
results/access_path/access_path_summary.md
...
Notes:
The temporary output files were removed from the working tree. Python `.pyc` cache files are still tracked in Git, but removing them with `git rm` requires writing `.git/index.lock`, which is blocked by the current sandbox. `.pytest_cache/` is not tracked and is now ignored by `.gitignore`, but recursive directory deletion was blocked by permission review timeout.

## Maintenance Note 026: Portable Project Export
...
```bash
# Copied selected files into portable_project/
# No experiment command was run.
```

Purpose:
Prepare a clean folder that can be moved to a new project location while preserving meaningful project content and excluding caches, temporary outputs, and Git metadata.
...
Included content:
- Root project files: `.gitignore`, `AGENTS.md`, `README.md`, `requirements.txt`, `STATUS.md`, `TODO.md`, `ROADMAP.md`, `DECISIONS.md`, `EXPERIMENT_LOG.md`.
- Source, configs, experiments, tests, docs, benchmark schema/example, and official `results/` artifacts.
...
Validation:
- Copied files: 149.
- `.pyc` files in export: 0.
- cache/temp directories in export: 0.

Notes:
This was a file migration/export task only. It did not modify Python source behavior, scheduler algorithms, model assumptions, config values, or generated official result artifacts.

"""Sweep FPP lane count on the 4-die stress workload."""
...
from src.visualize.sweep import plot_fpp_lane_sweep


CONFIG_PATH = ROOT / "configs" / "case_4die_stress.yaml"
RESULT_DIR = ROOT / "results" / "sweeps" / "fpp_lanes"
...
def summary_row(fpp_lanes: int, result: ScheduleResult) -> dict:
    """Return one sweep CSV row for a scheduler result."""
...
def build_results(case: dict) -> list[ScheduleResult]:
    """Run all three schedulers for one FPP lane setting."""
...
def case_with_fpp_lanes(base_case: dict, fpp_lanes: int) -> dict:
    """Return a deep-copied stress case with a modified FPP lane count."""

    if fpp_lanes <= 0:
        raise ValueError("fpp_lanes must be positive")
    case = copy.deepcopy(base_case)
    case["access"]["fpp_lanes"] = int(fpp_lanes)
    return case


def run(fpp_lanes_values: tuple[int, ...] = FPP_LANES) -> dict[str, Path]:
    """Run the FPP lane sweep and write summary CSV and SVG plots."""

    base_case = load_yaml(CONFIG_PATH)
    RESULT_DIR.mkdir(parents=True, exist_ok=True)

    rows: list[dict] = []
    for fpp_lanes in fpp_lanes_values:
        case = case_with_fpp_lanes(base_case, fpp_lanes)
        for result in build_results(case):
            rows.append(summary_row(fpp_lanes, result))

    summary_path = RESULT_DIR / "fpp_lane_sweep_summary.csv"
    write_dict_rows(summary_path, rows, SUMMARY_COLUMNS)
    outputs = {"fpp_lane_sweep_summary": summary_path}
    outputs.update(plot_fpp_lane_sweep(rows, RESULT_DIR))
    return outputs


if __name__ == "__main__":
    outputs = run()
    print("FPP lane sweep completed.")

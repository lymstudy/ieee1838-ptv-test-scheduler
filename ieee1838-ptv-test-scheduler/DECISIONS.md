...
## D26: B3.1.5 Layered Schedule Metrics Scope

B3.1.5 introduces a lightweight evaluator for schedules produced by the independent B3.1 `AccessTimeAwareScheduler`:

```text
src/layered/evaluator.py
```

The evaluator computes phase-level access/resource behavior only:

- PTAP busy time and utilization;
- FPP lane-seconds and utilization;
- DWR, capture, and local execution busy time;
- access overhead time and ratio;
- phase overlap metrics such as max and average parallelism;
- resource busy-time and phase-type duration summaries.

This evaluator intentionally does not call or modify the A0 schedule evaluator. It also does not change AccessPath estimation, B2 layered expansion, or B3.1 scheduler behavior.

B3.1.5 does not implement thermal or voltage prediction, predictive physical-aware scheduling, full IEEE 1838 compliance, or a complete IEEE 1838 framework. Thermal / voltage prediction integration remains future B3.2/B3.3 work.

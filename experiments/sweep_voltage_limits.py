"""Sweep IR-drop limit on the 4-die stress workload."""
...
from src.visualize.sweep import plot_voltage_limit_sweep


CONFIG_PATH = ROOT / "configs" / "case_4die_stress.yaml"
RESULT_DIR = ROOT / "results" / "sweeps" / "voltage_limits"
VOLTAGE_LIMITS = (0.10, 0.15, 0.20, 0.30, 0.50)
SUMMARY_COLUMNS = [
    "voltage_limit",
    "scheduler_name",
    "tat",
...
def summary_row(voltage_limit: float, result: ScheduleResult) -> dict:
    """Return one voltage-limit sweep row."""
...
def build_results(case: dict) -> list[ScheduleResult]:
    """Run all three schedulers for one voltage limit setting."""
...
def case_with_voltage_limit(base_case: dict, voltage_limit: float) -> dict:
    """Return a deep-copied stress case with a modified IR-drop limit."""

    if voltage_limit < 0:
        raise ValueError("voltage_limit must be non-negative")
    case = copy.deepcopy(base_case)
    case["voltage"]["max_ir_drop_v"] = float(voltage_limit)
    return case


def run(voltage_limits: tuple[float, ...] = VOLTAGE_LIMITS) -> dict[str, Path]:
    """Run the voltage-limit sweep and write summary CSV and SVG plots."""
    ...
    for voltage_limit in voltage_limits:
        case = case_with_voltage_limit(base_case, voltage_limit)
        for result in build_results(case):
            rows.append(summary_row(voltage_limit, result))

    summary_path = RESULT_DIR / "voltage_limit_sweep_summary.csv"
    write_dict_rows(summary_path, rows, SUMMARY_COLUMNS)
    outputs = {"voltage_limit_sweep_summary": summary_path}
    outputs.update(plot_voltage_limit_sweep(rows, RESULT_DIR))
    return outputs


if __name__ == "__main__":
    outputs = run()
    print("Voltage limit sweep completed.")

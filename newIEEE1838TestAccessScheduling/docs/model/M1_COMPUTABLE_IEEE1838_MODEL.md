...
- 能表示 die、core、PTAP、STAP、3DCR、DWR segment、FPP lane（optional, IEEE 1838-2019 Clause 7）、BIST、TSV/interposer route 和热邻接。
...
注意：FPP（Flexible Parallel Port）是 IEEE 1838-2019 标准的可选组件（Clause 7，optional），使用独立物理 lane，通过 TAP 配置。
...
| `fpp_channels` | FPP channel（optional, IEEE 1838-2019 Clause 7）|
| `fpp_lanes` | FPP lane（optional, IEEE 1838-2019 Clause 7）|
...
| `fpp_access` | 是否支持 FPP 访问（optional, IEEE 1838-2019 Clause 7）|
| `parallel_group` | 可并行或互斥分组 |
| `mode_config_bits` | 模式配置位开销 |

#### FPP Channel / Lane（optional, IEEE 1838-2019 Clause 7）
...
| `fpp_capacity_groups` | FPP lane 容量（optional, IEEE 1838-2019 Clause 7）|
...
### FPP 传输时间（IEEE 1838-2019 Clause 7, optional）
...
| `fpp_lanes.csv` | `lane_id` | `channel_id,direction,bandwidth_bps,registered,requires_clock_lane,mutual_exclusion_group`（optional, IEEE 1838-2019 Clause 7）|

...
- 受限 FPP bandwidth/lane（optional, IEEE 1838-2019 Clause 7），使 FPP 不是永远最优，而是作为可并行替代路径。
- 拓扑相关 BIST bank 数和热耦合参数，使 2.5D/3D/5.5D 不再只是换名字。
- 放大热 RC 参数，使 thermal proxy 能产生可比较的温度差异。

M21 仍然保留 IEEE 1838 访问语义：BIST 和 hybrid recipe 都需要先经过 PTAP/STAP 串行配置；
FPP（optional, IEEE 1838-2019 Clause 7）只表示批量数据传输通道，不表示任务可以绕过 TAP 启动。
...
> 在 ITC'02 派生的共享资源压力 benchmark 中，所有 recipe 仍先经过 PTAP/STAP 配置；固定选择每个 target 的最快访问路径会导致共享 BIST engine 串行化；联合 recipe 选择与调度能够在 TAP 启动之后混合 BIST 本地运行与 FPP（optional, IEEE 1838-2019 Clause 7）数据传输路径，在 12 个压力场景中获得 10.00% 到 46.67% 的测试时间收益。
